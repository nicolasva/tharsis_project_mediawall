...
class StatisticsController < ApplicationController
	...
	def show
		...
		@groupapp = Groupapp.find_by_name(params[:id])
	      else
	...
	def createnewcategoriecolonnetables
		...
		if @categoriecolonnetable.save
			flash[:notice] = t("controller.notice.statistics.createnewcategoriecolonnetables.positif") 
			redirect_to :controller=>"statistics", :action=>"categoriecolonne"
		else
			flash[:categoriecolonnetable] = @categoriecolonnetable
			flash[:notice] = t("controller.notice.statistics.createnewcategoriecolonnetables.negatif") 
			redirect_to(:back)
		end
	end
	...
	def destroystatistiquecategorie
		@categorieindex = Statistic::Categorieindex.find(params[:id])
		@categorieindex.destroy
		@categorieindex.posseders.destroy_all

		redirect_to(statistics_url) 
	end

	def updatestatisticsprincipal
		if flash[:categorieindex].nil?
			@categorieindex = Statistic::Categorieindex.find(params[:id])
		else
			@categorieindex = flash[:categorieindex]
	...
	def validupdatestatistiquesprincipal
	...
	def validstatistiquesprincipal
		...
		flash[:notice] = t("controller.notice.statistics.validstatistiquesprincipal.positif")
	     else
		flash[:notice] = t("controller.notice.statistics.validstatistiquesprincipal.negatif") 
		flash[:categorieindex] = @categorieindex
	     end
		redirect_to :controller=>"statistics", :action=>"statistiquesprincipalindex"
	end

	def get_stat_pie24h(colonne_name,colonne_id,time_now,time_24h,titregraphique)
		stat_graph_global_pie = StatGlobalPie.new(colonne_id,time_24h,time_now)
		tab_hash_total = stat_graph_global_pie.get_stat
		g = Gruff::Pie.new
		g.title = titregraphique
		compteur = 0
	...
	def valid_update_colonne_table
		...
		if @categoriecolonnetable.update_attributes(params[:categoriecolonnetable])
			flash[:notice] = t("controller.notice.statistics.valid_update_colonne_table.positif") 
			redirect_to :controller=>"statistics", :action=>"categoriecolonne"
		else
			flash[:categoriecolonnetable] = @categoriecolonnetable

			flash[:notice] = t("controller.notice.statistics.valid_update_colonne_table.negatif") 
			redirect_to(:back)
		end
	end
	...
	def valid_edit_colonne_table
		...
		@tab_affichagecolonneen = params[:edit_colonne_table][:affichagecolonneen]
		...
		@tab_apps_serial_column = params[:edit_colonne_table][:apps_serial_column]
		0.upto(@tab_colonne_id.length-1){ |i|		
			unless  Statistic::Colonne.update(@tab_colonne_id[i], :affichagecolonne=>@tab_affichagecolonne[i], :choixaffichage=>@tab_choixaffichage.include?(@tab_colonne_id[i]), :affichagecolonneen=>@tab_affichagecolonneen.include?(@tab_colonne_id[i]), :statx=>@tab_statx.include?(@tab_colonne_id[i]), :time_column=>@tab_time_column.include?(@tab_colonne_id[i]), :apps_serial_column=>@tab_apps_serial_column.include?(@tab_colonne_id[i]))
	...
	def stat_global_table(table_id,hash_date,application_serial,numerograph,titrelegendestatglobal)
		...
		time_max_comparate = Time.local(year_max,month_max,day_max,hour_max,minute_max)
			...
			g.data(titrelegendestatglobal, array_colonne_gruff)
	...
	def generategraph(tab_application,tab_table,tab_colonne,period,fromdate,todate,numerograph,choix_category,check_average_apps,check_total_apps)	
		...
		0.upto(tab_table.length-1){ |i|
			...
			unless choix_category.empty? && tab_application.nil?
				...
				0.upto(tab_application.length-1){ |i|
					application = Application.find(tab_application[i].to_i)
					@tab_image_visugraph.push(stat_global_table(@categoriecolonnetables.table_id,hash_date,application.serial,numerograph,@categoriecolonnetables.titrelegendestatglobal))
				}	
                          end
			end
	      end
