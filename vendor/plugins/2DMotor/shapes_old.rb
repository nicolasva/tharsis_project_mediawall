class String
  #@@ move this function to a better place, most probably String
  def xml_escape ()
    gsub(/[<&"']/) do |match|
      case match
      when '&': '&amp;'
...
module SVuGy
  ...
  class Shape
    ...
    def Shape.class_name_to_method_name (class_name) #@@@ should be in different place
      method_name = class_name.sub(/SVuGy::/,"")
    ...
    def Shape.inherited (subclass)
      ...
      unless AddShapes.method_defined? factory_method_name.to_sym
        # example:
        # def rect (*args, &block)
        #  add_new_shape Rect.new(self, *args, &block)
        # end
    ...
    def take_id (args)
      if args.length > 0 && args[0].kind_of?(Symbol)
        @id = args.shift
        register_id @id
        Rails.logger.info "-=-=-=-=-=-=-=-test affichage #{@id}-=-=-=-=-=-=-=-=-=-"
      end
    end
  ...
  class Circle < Shape
    def initialize (parent, *args, &block)
      ...
      para = args.shift    #@@@@ test circtle with radius only!      @r = para
      super(&block)
    end
  ...
  class Polyline < Shape
    ...
    def initialize (parent, tab, &block)	
    	#def take_point (args)
      	#	para = args.shift
      	#	if para.respond_to?(:to_point)
        #		x, y = para.to_point.local(self).to_a
