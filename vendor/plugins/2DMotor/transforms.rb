module SVuGy
  module AddTransforms
    ...
    ## need to objectify transform collection before this works
    # def transform (&block)
    # end
  ...
  class Translate < Transform
    ...
    def to_svg
      if @ty && @ty!=0 #@@ maybe also check for 'compact' in the @ty!=0 case
        "translate(#{@tx} #{@ty})"
      else
        "translate(#{@tx})"
      end
  ...
  class Scale < Transform
    ...
    def to_svg
      if @sy && @sx!=@sy #@@ maybe also check for 'compact' in the @sx!=@sy case
        "scale(#{@sx} #{@sy})"
      else
        "scale(#{@sx})"
      end
