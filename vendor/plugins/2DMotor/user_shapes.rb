module SVuGy
  ...
  class BentArrow < BentLine 
    def initialize (parent, *args, &block)   #@@@@ extremely similar to Arrow
      super(parent, *args, &block)
      if false #@@@@ not supported in Opera (v. 9.20/9.21)
    ...
    def to_svg
      result = super
      if true #@@@@ create arrowhead by hand, to avoid Opera problem
  ...
  class GridLine < Shape
    ...
    def to_svg
      ...
      result << "</g>\n"
  #@@@@These 4lines don't working on Firefox.
  #    result = "<pattern id='grid' width='#{@distance}' height='#{@distance}' patternUnits='userSpaceOnUse'>\n"
  #    result << "<rect x='0' y='0' width='#{@distance}' height='#{@distance}' fill='none'#{common_attrs}/>\n"
  #    result << "</pattern>\n"
  #    result << "<rect x='0' y='0' width='100%' height='100%' fill='url(#grid)'#{common_attrs}/>\n"
    end
  end
  ...
  #   if @yg > @ys then @sweep = 0 end
    ...
    def to_svg
  #    "<path d='M #{start} L #{root_s} #{pass.join(" ")} #{root_g} Z'#{common_attrs}/>\n"
  ...
  class RotatedText < Text
    def initialize (*args, &block)
      angle = args.pop
      super(*args, &block)
      # unclear whether 'rotate' should come before or after 'super';
      # depends on how other transformations work
      rotate angle, @x, @y
    end
  end
